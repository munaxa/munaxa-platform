/**
 * Elevation (box-shadow) tokens — the shared shadow *geometry*, in five steps: sm · md · lg ·
 * xl · 2xl. The colour of a shadow is a brand value: it is read from the active theme's
 * `--shadow-tint` / `--glow-tint` custom properties, so the same ramp carries each product's
 * own tint.
 */
export const elevation = {
  ...
  xl: '0 20px 44px rgb(var(--shadow-tint) / 0.18)',
  '2xl': '0 32px 68px rgb(var(--shadow-tint) / 0.22)',
